...
typedef struct Gif_ReaderBuffer {
  ...
  int is_record;
  int is_eoi;
  uint8_t (*byte_getter)(struct Gif_ReaderBuffer*);
  uint32_t (*block_getter)(uint8_t*, uint32_t, struct Gif_ReaderBuffer*);
  int (*eofer)(struct Gif_ReaderBuffer*);
} Gif_ReaderBuffer;
...
static uint32_t
buffer_block_getter(uint8_t *p, uint32_t s, Gif_ReaderBuffer *grr)
{
    if ((grr->pos + s) > grr->size)
    {
      uint32_t count = grr->size - grr->pos;
      memcpy(p, grr->buffer + grr->pos, count);
      memset(p + count, 0, s - count);
      grr->pos += count;
      return count;
    }
    else
    {
      memcpy(p, grr->buffer + grr->pos, s);
      grr->pos += s;
      return s;
    }
}
...
static int
buffer_eofer(Gif_ReaderBuffer *grr)
{
  if (grr->pos >= grr->size)
    return 1;
  else 
...
static uint8_t
buffer_record_byte_getter(Gif_ReaderBuffer *grr)
{
...
static uint32_t
buffer_record_block_getter(uint8_t *p, uint32_t s, Gif_ReaderBuffer *grr)
{
...
static int
buffer_record_eofer(Gif_ReaderBuffer *grr)
{
...
static inline uint16_t
buffer_gifgetunsigned(Gif_ReaderBuffer *grr)
{
  uint8_t one = gifgetbyte(grr);
  uint8_t two = gifgetbyte(grr);
  return one | (two << 8);
...
static void
make_data_reader_buffer(Gif_ReaderBuffer *grr, const uint8_t *data, uint32_t length)
{
  grr->v = data;
  grr->pos = 0;
  grr->length = length;
  grr->is_record = 1;
  grr->byte_getter = buffer_record_byte_getter;
  grr->block_getter = buffer_record_block_getter;
  grr->eofer = buffer_record_eofer;
}
...
static int
read_image_block_buffer(Gif_ReaderBuffer *grr, uint8_t *buffer, int *bit_pos_store,
...
static void
read_image_data_buffer(Gif_Context *gfc, Gif_ReaderBuffer *grr)
{
...
#define CUR_CODE_MASK ((1 << bits_needed) - 1)
  ...
  while (1) {
    ...
    if (bit_position + bits_needed > bit_length)
      /* Read in the next data block. */
      if (!read_image_block_buffer(grr, buffer, &bit_position, &bit_length,
                            bits_needed))
...
static Gif_Colormap *
read_color_table_buffer(int size, Gif_ReaderBuffer *grr)
{
...
static int
read_logical_screen_descriptor_buffer(Gif_Stream *gfs, Gif_ReaderBuffer *grr)
     /* returns 0 on memory error */
{
  uint8_t packed;

  /* we don't care about logical screen width or height */
  gfs->screen_width = buffer_gifgetunsigned(grr);
  gfs->screen_height = buffer_gifgetunsigned(grr);
  ...
  if (packed & 0x80) { /* have a global color table */
      ...
      gfs->global = read_color_table_buffer(ncol, grr);
...
static int
read_compressed_image_buffer(Gif_Image *gfi, Gif_ReaderBuffer *grr, int read_flags)
{
  if (grr->is_record) {
...
static int
uncompress_image_buffer(Gif_Context *gfc, Gif_Image *gfi, Gif_ReaderBuffer *grr)
{
    int old_nerrors;
    ...
    read_image_data_buffer(gfc, grr);
...
static int
read_image_buffer(Gif_ReaderBuffer *grr, Gif_Context *gfc, Gif_Image *gfi, int read_flags)
     /* returns 0 on memory error */
{
  uint8_t packed;

  gfi->left = buffer_gifgetunsigned(grr);
  gfi->top = buffer_gifgetunsigned(grr);
  gfi->width = buffer_gifgetunsigned(grr);
  gfi->height = buffer_gifgetunsigned(grr);
  ...
  if (packed & 0x80) { /* have a local color table */
    int ncol = 1 << ((packed & 0x07) + 1);
    gfi->local = read_color_table_buffer(ncol, grr);
  ...
  if (read_flags & GIF_READ_COMPRESSED) {
    if (!read_compressed_image_buffer(gfi, grr, read_flags))
      return 0;
    if (read_flags & GIF_READ_UNCOMPRESSED) {
      Gif_ReaderBuffer new_grr;
      make_data_reader_buffer(&new_grr, gfi->compressed, gfi->compressed_len);
      if (!uncompress_image_buffer(gfc, gfi, &new_grr))
        return 0;
    }

  } else if (read_flags & GIF_READ_UNCOMPRESSED) {
    if (!uncompress_image_buffer(gfc, gfi, grr))
      return 0;
...
static void
read_graphic_control_extension_buffer(Gif_Context *gfc, Gif_Image *gfi,
                               Gif_ReaderBuffer *grr)
{
  uint8_t len;
  ...
  if (len == 4) {
    ...
    gfi->delay = buffer_gifgetunsigned(grr);
...
static char *
suck_data_buffer(char *data, int *store_len, Gif_ReaderBuffer *grr)
{
...
static int
read_unknown_extension_buffer(Gif_Context* gfc, Gif_ReaderBuffer* grr,
...
static int
read_application_extension_buffer(Gif_Context *gfc, Gif_ReaderBuffer *grr)
{
  ...
  if (len == 11
    ...
    if (len == 3) {
      ...
      gfs->loopcount = buffer_gifgetunsigned(grr);
      len = gifgetbyte(grr);
      if (len)
    ...
    return read_unknown_extension_buffer(gfc, grr, 0xFF, (char*)buffer, len);
}
...
static int
read_comment_extension_buffer(Gif_Image *gfi, Gif_ReaderBuffer *grr)
{
  int len;
  Gif_Comment *gfcom = gfi->comment;
  char *m = suck_data_buffer(0, &len, grr);
  if (m) {
    if (!gfcom)
...
static Gif_Stream *
read_gif_buffer(Gif_ReaderBuffer *grr, int read_flags,
         const char* landmark, Gif_ReadErrorHandler handler)
{
  ...
  GIF_DEBUG(("\nGIF "));
  if (!read_logical_screen_descriptor_buffer(gfs, grr))
    goto done;
    ...
    switch (block) {
     ...
     case ',': /* image block */
      ...
      if (!Gif_AddImage(gfs, gfi))
          goto done;
      else if (!read_image_buffer(grr, &gfc, gfi, read_flags)) {
     ...
     case '!': /* extension */
      ...
      switch (block) {
       ...
       case 0xF9:
        read_graphic_control_extension_buffer(&gfc, gfi, grr);
        break;

       case 0xCE:
        last_name = suck_data_buffer(last_name, 0, grr);
        break;

       case 0xFE:
        if (!read_comment_extension_buffer(gfi, grr)) goto done;
        break;

       case 0xFF:
        read_application_extension_buffer(&gfc, grr);
        break;

       default:
        read_unknown_extension_buffer(&gfc, grr, block, 0, 0);
        break;
                 ...
                 const char* landmark, Gif_ReadErrorHandler h)
{
  ...
  grr.pos = 0;
  grr.is_record = 0;
  grr.byte_getter = buffer_byte_getter;
  grr.block_getter = buffer_block_getter;
  grr.eofer = buffer_eofer;
  return read_gif_buffer(&grr, read_flags, landmark, h);
}
